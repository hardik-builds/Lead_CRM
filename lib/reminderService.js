...
// Robust Indian Date Normalizer: Converts ANY date format to DD/MM/YYYY (Indian Standard)
export function normalizeToIndianDate(val) {
  ...
  // 2. Match DD-MM-YYYY or DD/MM/YYYY or DD.MM.YYYY (Indian DD/MM/YYYY format)
  const dmyMatch = str.match(/^(\d{1,2})[-/. ](\d{1,2})[-/. ](\d{4})/);
  if (dmyMatch) {
    let p1 = parseInt(dmyMatch[1], 10); // Day in Indian DD/MM/YYYY mode
    let p2 = parseInt(dmyMatch[2], 10); // Month
    let year = dmyMatch[3];
    let day = p1;
    let month = p2;

    // Safety swap if p1 <= 12 and p2 > 12 (e.g. 07/28/2026 -> MM/DD/YYYY)
    if (p1 <= 12 && p2 > 12) {
      day = p2;
      month = p1;
    }
